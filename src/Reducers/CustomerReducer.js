...
export const customerReducer = (state, action) => {
  ...
  const getServices = (withOilOptions = false) => {
    axios
      .get(
        `${Helpers.baseUrl}services/all/${Helpers.authParentId}`,
        Helpers.headers
      )
      .then((response) => {
        ...
        for (let index = 0; index < resData.length; index++) {
          ...
          const serviceOption = {
            label:
              resData[index].service_name + " ($" + resData[index].price + ")",
            value: service,
          };
  ...
  const getTires = () => {
    axios
      .get(
        `${Helpers.baseUrl}tires/all/${Helpers.authParentId}`,
        Helpers.headers
      )
      .then((response) => {
        let resData = response.data.reverse();
  ...
  const getOils = () => {
    axios
      .get(
        `${Helpers.baseUrl}oils/all/${Helpers.authParentId}`,
        Helpers.headers
      )
      .then((response) => {
        let resData = response.data.reverse();
        for (let index = 0; index < resData.length; index++) {
          const service = {
            ...
            name:
              ...
              ")",
            price: response.data[index].pricePerQuartz,
          ...
          const oilOption = {
            label:
              ...
              response.data[index].type +
              ") ( $" +
              response.data[index].pricePerQuartz +
              " )",
            value: service,
          };
  ...
  switch (action.type) {
    case "getCustomers":
      axios
        .get(
          `${Helpers.baseUrl}customers/all/${Helpers.authParentId}`,
          Helpers.headers
        )
        .then((response) => {
          let customerArray = [];
    ...
    case "getCustomerVehicles":
      axios
        .get(
          `${Helpers.baseUrl}vehicles/customer/${action.data.customerId}`,
          Helpers.headers
        )
        .then((response) => {
          let vehiclesArray = [];
          ...
          for (let index = 0; index < resData.length; index++) {
            const object = {
              label: resData[index].name + " " + resData[index].model + "",
              value: resData[index],
            };
    ...
    case "getTax":
      axios
        .get(
          `${Helpers.baseUrl}customers/get-tax/${Helpers.authParentId}`,
          Helpers.headers
        )
        .then((response) => {
    ...
    case "getTechnicians":
      axios
        .get(
          `${Helpers.baseUrl}employees/all/${Helpers.authParentId}`,
          Helpers.headers
        )
        .then((response) => {
